...
const getTitle = async (title) => {
    //find movie id number from search title
    const searchedMovieResponse = await fetch(`${searchMovieURL}${API_KEY}&language=en-US&query=${title}&page=1&include_adult=false`);
    const searchedMovieData = await searchedMovieResponse.json();
    const movieId = searchedMovieData.results[0].id

    //with movie id - find movie - find cast and crew
   ...
   return console.log("Cast: ", castMembers, 'Director: ', directorName)
  }; 
